...
exports.validateUser = function validateUser(user) {
  console.log('validating users from database');
    db.all('select * from users', function (err, rows) {
      for (var i = 0; i < rows.length; i++) {
        var row = rows[i];
        console.log('  name: ' + row['sname'] + ', age: ' + row['age']);
      }
      cb(null);
    });
...
exports.addWarbs = function(user, date, warb, attch){
  warbledb.push(new Warble (user, date, warb ,attch, 'none'));
}
